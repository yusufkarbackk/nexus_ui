    ...
    >
      ...
      >
        <div className="p-2 rounded-lg bg-indigo-100">
          <Send className={`w-5 h-5 ${categoryStyle.color}`} strokeWidth={2} />
        </div>
        <div className="flex-1 min-w-0">
          <h3 className={`font-semibold text-sm ${categoryStyle.color} truncate`}>
            {data.application?.name || data.label}
          </h3>
          <p className="text-[10px] text-slate-500 truncate">
            {data.application?.appKey}
          </p>
        </div>
        <div className="px-2 py-0.5 rounded-full text-[10px] font-medium uppercase tracking-wide bg-indigo-200 text-indigo-700">
          Source
        </div>
      </div>
      ...
      <Handle
        ...
        className="!w-3 !h-3 !bg-indigo-400 !border-2 !border-white hover:!bg-indigo-600 transition-colors"
      />
    </div>
  );
}
